...
const commonFieldProps = computed(() => {
    return {
        name: "total-price",
        label: t("debit_rows.fields.Total price"),
...
const { value, decimalPlaces, valueIsManual } = useNumberFieldValue({
    ...
    valueAttribute: "totalPrice",
    precisionAttribute: "totalPricePrecision",
    decimalPlaces: 8,
    manualAttribute: "totalPriceIsManual",
});
...
const { errors } = useOrderFormFieldErrors({
    recordAccessor: getDebitRow,
    fields: ["total_price", "total_price_precision"],
});
