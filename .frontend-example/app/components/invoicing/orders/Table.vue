<template lang="pug">
.invoicing-orders(v-if="selectedView")
    NDataTable(
        ref="table"
...
const rowsStore = useInvoicingOrdersRowsStore();
...
const columnsStore = useInvoicingOrdersColumnsStore();
...
const tableColumns = computed(() => {
    const cols = columns.value.map(col => ({
        key: col.key,
        title: t("invoicing.orders.columns." + col.key),
        ...
        ellipsis: {
            tooltip: {
                contentClass: "invoicing-orders-row-ellipsis",
            }
        },
        ellipsisComponent: "performant-ellipsis",
        render: getTableColumnRenderFunction(col, INVOICING_ORDER_ROW_DEFINITION),
    })) as DataTableColumn[];
...
const getRowProps = (row: InvoicingOrderTableRow) => {
    ...
    const openEditOrderForm = (e: MouseEvent) => {
        ...
        goToOrderRoute({ id: row.id, tab: OrderFormTabName.DebitRows });
    };
