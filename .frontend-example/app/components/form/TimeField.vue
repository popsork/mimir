...
// value model strings are expected (and emitted) in HH:MM:SS format
...
// when the user is, for example, still only adjusting the time by clicking on the hour and minute arrows.
...
const convertInternalValueToModelValue = (input: Date) => {
    const neededSeconds = (input.getMinutes() === 59) ? 59 : 0;
    input.setSeconds(neededSeconds, 0);
...
const handleShow = () => {
    ...
    if (internalValue.value === null && picker.value) {
