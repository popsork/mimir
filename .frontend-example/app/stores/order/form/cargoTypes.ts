import { CargoType, type CargoTypeApiResponseResource } from "~/models/CargoType";

export const useOrderFormCargoTypesStore = defineStore("order-form-cargo-types", () => {
    const model = CargoType;
    const repo = useRepo(model);
    const waiterName = WaitingFor.OrderFormCargoTypes;
    ...
    const fetchRecords = wrapFunctionInApiErrorHandler(async () => {
        const apiResponse: { data: CargoTypeApiResponseResource[] } = await useApi().getCargoTypes();
    ...
    return {
        loadCargoTypesIfNeeded: loadRecordsIfNeeded,
        cargoTypes: records,
        waitingForCargoTypes: waitingForRecords,
    };
});
