import { Line, type LineApiResponseResource } from "~/models/Line";

export const useOrderFormLinesStore = defineStore("order-form-lines", () => {
    const model = Line;
    const repo = useRepo(model);
    const waiterName = WaitingFor.OrderFormLines;
    ...
    const fetchRecords = wrapFunctionInApiErrorHandler(async () => {
        const apiResponse: { data: LineApiResponseResource[] } = await useApi().getLines();
    ...
    return {
        loadLinesIfNeeded: loadRecordsIfNeeded,
        lines: records,
        waitingForLines: waitingForRecords,
    };
});
