import { DocumentTypeModel, type DocumentTypeApiResponseResource } from "~/models/DocumentTypeModel";

export const useOrderFormDocumentTypesStore = defineStore("order-form-document-types", () => {
    const model = DocumentTypeModel;
    const repo = useRepo(model);
    const waiterName = WaitingFor.OrderFormDocumentTypes;
    ...
    const fetchRecords = wrapFunctionInApiErrorHandler(async () => {
        const apiResponse: { data: DocumentTypeApiResponseResource[] } = await useApi().getDocumentTypes();
    ...
    return {
        loadDocumentTypesIfNeeded: loadRecordsIfNeeded,
        documentTypes: records,
        waitingForDocumentTypes: waitingForRecords,
    };
});
