import { QuantityUnit, type QuantityUnitApiResponseResource } from "~/models/QuantityUnit";

export const useOrderFormQuantityUnitsStore = defineStore("order-form-quantity-units", () => {
    const model = QuantityUnit;
    const repo = useRepo(model);
    const waiterName = WaitingFor.OrderFormQuantityUnits;
    ...
    const fetchRecords = wrapFunctionInApiErrorHandler(async () => {
        const apiResponse: { data: QuantityUnitApiResponseResource[] } = await useApi().getQuantityUnits();
    ...
    return {
        loadQuantityUnitsIfNeeded: loadRecordsIfNeeded,
        quantityUnits: records,
        waitingForQuantityUnits: waitingForRecords,
    };
});
