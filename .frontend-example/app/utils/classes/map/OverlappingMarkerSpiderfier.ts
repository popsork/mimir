/**
 * source copied and adapted from https://github.com/chancezeus/oms/tree/master/lib
 *
 * copied files
 *
 * lib/oms-types.ts
...
export interface SpiderOptions {
    ...
    legWeight?: number;
    ignoreZoom?: boolean; // custom option
}
...
const MAX_ZINDEX: number = 100000;
...
export class OverlappingMarkerSpiderfier implements SpiderOptions {
    private readonly spiderfiedZIndex: number = MAX_ZINDEX + 20000;
    private readonly highlightedLegZIndex: number = MAX_ZINDEX + 10000;
    private readonly usualLegZIndex: number = MAX_ZINDEX + 1;
    ...
    private formatTimeoutId: number | null;
    private formatIdleListener: google.maps.MapsEventListener | null;
    ...
    ignoreZoom: boolean = false; // custom option
    ...
    private markerChangeListener(marker: any, positionChanged: boolean) {
        ...
        if (marker["_omsData"] && (positionChanged || !marker.map)) {
    ...
    private static minExtract(set: any[], callback: (item: any) => number) {
        ...
        let bestIndex = null as null | number;
        let bestValue = null as null | number;
    ...
    addMarker(marker: google.maps.marker.AdvancedMarkerElement, spiderClickHandler?: Function): OverlappingMarkerSpiderfier {
        //marker.map = this.map; // custom code change, marker should be on the map already
    ...
    removeMarker(marker: any) {
        ...
        // marker.map = null; // custom code change, let the initial caller remove the marker from the map
    ...
    markersNearMarker(marker: any, firstOnly: boolean = false) {
        ...
        for (let i = 0; i < this.markers.length; i++) {
            ...
            if (current === marker || current.map == null || !current.map) {
                continue;
            }
    ...
    constructor(private map: google.maps.Map, options: SpiderOptions = {}) {
        ...
        this.projectionHelper = new (class extends (google.maps.OverlayView as {
            new(): any;
            draw(): void;
        ...
        if (!this.ignoreZoom) {  // custom code change
