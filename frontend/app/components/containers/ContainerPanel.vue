...
import { useContainerMetricsStore } from '~/stores/containers/metrics';
...
const getMetric = (name: string) => metrics.value.find((metric: any) => metric.metric === name);
...
const updatedAt = computed(() => {
  const timestamps = metrics.value.map((metric: any) => metric.timestamp).filter(Boolean);
...
const statusLabel = computed(() => {
  ...
  return statusValue.value ? 'Running' : 'Stopped';
});
...
<template>
  <div class="space-y-6">
    <UCard>
      <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
          <UBadge :color="statusBadgeColor(statusValue)">
            {{ statusLabel }}
          </UBadge>
    ...
    <div v-if="loading" class="text-center text-muted">Loading metrics...</div>
